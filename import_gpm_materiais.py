# -*- coding: utf-8 -*-
"""
CSV Materiais por Projeto (usando lista do arquivo modelo_importar_obras_ponto sempre)
...
CSV_FINAL_NAME = "modelo_materiais_lote.csv"

MATERIAIS_SHEET = "MATERIAIS_POR_PONTO"
RANGE_COL_I = f"{MATERIAIS_SHEET}!I2:I"
RANGE_COL_C = f"{MATERIAIS_SHEET}!C2:C"
    ...
    for attempt in range(1, tries + 1):
        try:
            ...
            if status in (429, 500, 502, 503, 504):
                ...
                print(f"⚠️ HTTP {status} (tentativa {attempt}/{tries}) — aguardando {sleep_s:.1f}s...")
                time.sleep(sleep_s)
                last_err = e
                continue
            raise
            ...
            last_err = e
            sleep_s = base_sleep * (2 ** (attempt - 1)) + random.random() * jitter
            print(f"⚠️ Erro '{type(e).__name__}' (tentativa {attempt}/{tries}) — aguardando {sleep_s:.1f}s...")
            time.sleep(sleep_s)
    raise last_err
...
def drive_upload_or_update_unique_csv(drive_service, folder_id: str, filepath: str, filename: str) -> str:
    ...
    if files:
        keep = files[0]
        keep_id = keep["id"]
        print(f"♻️ Atualizando arquivo existente no Drive (overwrite): {filename} | id={keep_id}")
        ...
        try:
            ...
            print(f"✅ Atualizado. Link: {updated.get('webViewLink')}")
            kept_id = updated["id"]
        except HttpError as e:
            status = getattr(e.resp, "status", None)
            print(f"⚠️ Não consegui ATUALIZAR (HTTP {status}). Vou criar novo.")
            kept_id = None

    if not kept_id:
        print(f"☁️ Criando novo arquivo no Drive: {filename}")
        ...
        print(f"✅ Criado. Link: {created.get('webViewLink')}")
        kept_id = created["id"]
    ...
    if extras:
        print(f"🧹 Encontrados {len(extras)} duplicados com o mesmo nome. Tentando remover...")
        for f in extras:
            fid = f["id"]
            mtime = f.get("modifiedTime")
            size = f.get("size")
            print(f"   - removendo duplicado: id={fid} | modifiedTime={mtime} | size={size}")
            try:
                result = retry(lambda: _drive_try_delete_or_trash(drive_service, fid))
                if result == "deleted":
                    print("     ✅ removido")
                elif result == "trashed":
                    print("     ✅ enviado para lixeira")
                else:
                    print("     ⚠️ 404/sem acesso — não removido, seguindo")
            except HttpError as e:
                status = getattr(e.resp, "status", None)
                print(f"     ❌ erro removendo duplicado (HTTP {status}): {e}")
...
def ler_planilhas_origem_bd_config(sheets_service):
    print(f"🔄 Lendo lista de planilhas em {BD_CONFIG_RANGE_SOURCES} (planilha 1Ipp...)...")
    ...
    print(f"✅ Planilhas para consultar (únicas): {len(unique)}")
    return unique


def coletar_materiais_em_planilha(sheets_service, spreadsheet_id: str):
    ranges = [RANGE_COL_I, RANGE_COL_C, RANGE_COL_E]
    vrs = read_batch_columns(sheets_service, spreadsheet_id, ranges)

    col_i = vrs[0].get("values", []) if len(vrs) > 0 else []
    col_c = vrs[1].get("values", []) if len(vrs) > 1 else []
    col_e = vrs[2].get("values", []) if len(vrs) > 2 else []

    max_len = max(len(col_i), len(col_c), len(col_e))

    mapa = {}
    for idx in range(max_len):
        proj = get_val(col_i, idx)
        if not proj:
            continue
        material = get_val(col_c, idx)
        qtd = get_val(col_e, idx)
        mapa.setdefault(proj, []).append((material, qtd))
...
def main():
    ...
    print("🔎 Localizando no Drive a lista de projetos (modelo_importar_obras_ponto)...")
    ...
    print("\n✅ Lista escolhida:")
    print(f"   Nome: {rel_name}")
    print(f"   Modificado: {rel_mtime}")
    print(f"   FileID: {rel_id}")
    ...
    print("\n🔎 Consultando MATERIAIS_POR_PONTO nas planilhas listadas...")
    ...
    set_projetos = set(projetos)

    for idx, sid in enumerate(planilhas_origem, start=1):
        print(f"   ➜ ({idx}/{len(planilhas_origem)}) Planilha: {sid}")
        try:
            mapa = coletar_materiais_em_planilha(sheets_service, sid)
            planilhas_ok += 1

            for proj in set_projetos.intersection(mapa.keys()):
                for (material, qtd) in mapa[proj]:
                    linhas_csv.append([proj, material, qtd])
                    encontrados_total += 1
                    projetos_encontrados.add(proj)
    ...
    if nao:
        print(f"\n⚠️ Projetos NÃO encontrados em nenhuma planilha ({len(nao)}):")
        for p in nao[:60]:
            print(f" - {p}")
        if len(nao) > 60:
            print(f" ... (+{len(nao) - 60} outros)")
