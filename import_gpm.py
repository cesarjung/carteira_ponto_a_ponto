...
2) Exclui obras não aptas conforme Carteira:
   ...
   - Carteira!AX vazia ou "-"
   - Carteira!AY vazia ou "-"
...
def read_carteira_base9_excluidas(sheets_service):
    ...
    for i in range(len(col_b)):
        ...
        if coordenada_invalida(lat):
            motivos.append("Carteira!AX sem latitude ou com '-'")

        if coordenada_invalida(lon):
            motivos.append("Carteira!AY sem longitude ou com '-'")
...
def montar_primeiro_csv():
    ...
    print("🔄 Lendo base9 excluídas (OBRA RETIRADA, CONCLUÍDA, sem AX/AY ou com '-')...")
    ...
    print(f"   Linhas removidas por filtros D/U/AX/AY vazios ou '-': {removidos_excluidas}")
